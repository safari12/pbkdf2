defmodule PBKDF2 do
  ...
  First parameter is mnemonic phrase
  Second parameter is the salt
  Third parameter is the PBKDF2 key-stretching function is the options
  using 2048 rounds of hashing with the HMAC-SHA512 algorithm,
  producing a 512-bit value as its final output, which is the seed.
  ...
  * `:digest`     - an hmac function to use as the pseudo-random function
                    Defaults to `:sha512`
  """
  @spec generate(String.t, String.t, list) :: String.t
  def generate(secret, salt, opts \\ []) do
    ...
    generate(mac_func(opts[:digest]), secret, salt, opts, 1, [])
  end
